...
`PcmAudio.Sample(frame, channel)` validates both coordinates and throws `ArgumentOutOfRangeException` for invalid access. WAV serialisation rejects samples outside their signed bit-depth range, format fields that cannot be represented, and output larger than the maximum byte array length with `InvalidOperationException`. These checks also cover edits to the shared sample array. Validation failures leave destination files and streams unchanged; I/O failures during writing can still leave partial output. Keep sample and metadata buffers stable during serialisation. PCM APIs and audio helpers now carry nullable annotations; helpers reject null payloads and keys before reading input files or consuming streams.
...
`VideoCoding` captures `Spread` before invoking frame callbacks. Capacity totals and proportional allocation use integer arithmetic that supports the full nonnegative `long` range of individual frame capacities; reported capacity saturates at `long.MaxValue` after subtracting the video header. Negative frame counts or capacities and null extracted payloads raise `InvalidOperationException`. Length feasibility is a capacity check; embedding still requires a payload plus its header to fit in a byte array and available memory. Video helpers reject null payloads and keys before opening input files.

Video embedding checks capacities and piece feasibility before modifying frames in order. An embedding failure can leave earlier frames changed; the failing frame follows the sequence and frame algorithm's contracts, and later frames remain untouched. `FrameList` shares its caller's list and frame objects, so changes inside a failing callback remain visible. Its indexer now applies the same range and null-frame checks as its callback methods. AVI sequences and `ImageSequence` store decoded frames after callbacks succeed; image-file writes can still fail partway through. `ImageSequence.Paths` is a read-only copy of the supplied path list. Keep frame count, order and inner algorithm configuration stable during operations, and use independent storage for embedding targets. Frame references passed to callbacks must not be retained or disposed.

`AviVideo.AddFrame(byte[])` and `SetFrameData` copy input buffers, and `GetFrameData` returns an independent copy. To edit raw frame bytes, retrieve them, change the copy, then call `SetFrameData`. Typed image and JPEG methods also store independent encoded frames; decoded images belong to the caller. Typed JPEG insertion requires an MJPEG container and matching dimensions. Replacement methods validate the frame index before encoding. Rejected arguments and failed sequence callbacks preserve stored frame data. Raw RGB insertion checks frame size; raw MJPEG insertion and container loading check the JPEG signature, with JPEG contents and dimensions validated on decoding. AVI APIs now carry nullable annotations. Keep input images and video state stable during operations and synchronise concurrent access externally.
